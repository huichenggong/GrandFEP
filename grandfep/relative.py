...
def invert_dict(dictionary: dict) -> dict:
    """
    Invert the value and key of a dictionary.
    ...
    * Implement omitted terms (this was not available in the original class).
    ...
    def __init__(self,
        ...
        interpolate_old_and_new_14s : bool, default False
            ...
            they are present in the nonbonded force. Not implemented
        """
...
class HybridTopologyFactoryREST2:
    """
    ...
    All the alchemically changed atoms (core, unique_old, unique_new) will be considered "hot" in REST2.
    Other atoms in the environment can also be added by `old_rest2_region_atom_indices`
    ...
    Here are the forces in the hybrid system:
        ...
        - CustomTorsionForce
            handles torsions involving `core_atoms`, `rest2_atoms`, `unique_old_atoms` and `unique_new_atoms` (these are interpolated).
            We are planning to scale d-d-d-d, r-d-d-d and r-r-d-d to 0 in the future
        - PeriodicTorsionForce
        ...
        - CustomBondForce_exceptions
            handles all electrostatics and sterics exceptions involving unique old/new atoms when
    ...
    def __init__(self, old_system, old_positions, old_topology,
        ...
        # Other options
        self._use_dispersion_correction = use_dispersion_correction
        self._interpolate_14s = False # interpolate_old_and_new_14s

        # Sofcore options
        self._softcore_alpha = softcore_alpha
        minmax = (0,1)
        if softcore_alpha < minmax[0] or softcore_alpha > minmax[1]:
            raise AssertionError(f"{softcore_alpha=} is not in {minmax}")
        ...
        # Add box
        self._handle_box() # barostat will be ignored
        ...
        # add bonded terms
        self._add_bond_force_terms()    # This REST2 does not change bonds
        self._add_angle_force_terms()   # This REST2 does not change angles
        self._add_torsion_force_terms() # REST2 scales h-h-h-h, h-h-h-c, h-h-c-c, h-c-c-c, torsions
    ...
    def _set_mappings(self, old_to_new_map, core_old_to_new_map):
        ...
        * For now this directly sets the system, core and env old_to_new_map,
        ...
        self._old_to_new_map = old_to_new_map
        self._core_old_to_new_map = core_old_to_new_map
        self._new_to_old_map = invert_dict(old_to_new_map)
        self._core_new_to_old_map = invert_dict(core_old_to_new_map)
        ...
        self._env_new_to_old_map = invert_dict(self._env_old_to_new_map)
    ...
    def _add_particles(self):
        ...
        * If there is a difference in masses between the old and new systems
        ...
        # Create the opposite atom maps for later use (nonbonded processing)
        self._hybrid_to_old_map = invert_dict(self._old_to_hybrid_map)
        self._hybrid_to_new_map = invert_dict(self._new_to_hybrid_map)

    def _handle_box(self):
        """
        Copies over the box vectors as necessary.
        """

        box_vectors = self._old_system.getDefaultPeriodicBoxVectors()
        self._hybrid_system.setDefaultPeriodicBoxVectors(*box_vectors)

    def _set_atom_classes(self, old_rest2_atom_indices):
        """
        This method determines whether each atom belongs to unique old,
        unique new, core, or environment, as defined in the class docstring.
        All indices are indices in the hybrid system.
    ...
    def _add_torsion_force_terms(self):
        """
        This function adds a CustomTorsionForce terms to the system. All the REST2 atoms are
        interpolated (h-h-h-h, h-h-h-c, h-h-c-h, h-c-h-h, c-h-h-h, h-h-c-c, h-c-h-c, ....).
        This will normally include Core and unique atoms.
    ...
    def _add_nonbonded_force_terms(self):
        ...
        sterics_custom_nonbonded_force.addGlobalParameter(
            "lambda_sterics_delete", 0.0)
        sterics_custom_nonbonded_force.addGlobalParameter(
            "k_rest2_sqrt", 1.0) # sqrt(T_cold / T_hot)
    ...
    def classify_torsion(old_torsion_dict, new_torsion_dict):
        ...
        itersect_torsion : set
            ...
            torsions that appear in both old and new systems
    ...
    @staticmethod
    def _torsion_str(is_old:list=None, is_new:list=None, is_hot:list=None, hybird:bool=True, torsion_type:str=None,
                     scaling:float=1.0, rest2_n_is_hot:int=0):
        """
        ...
        # only one of is_old and is_new is needed
        if is_old is not None and is_new is None:
            t_string = ("_".join(f"{'O' if old else "R"}{'h' if hot else 'c'}" for old, hot in zip(is_old, is_hot))
                        + "_" + torsion_type + f"_{h_str}_{sum(is_hot)}_{scaling:.2f}_{rest2_n_is_hot}")
        elif is_old is None and is_new is not None:
            t_string = ("_".join(f"{'N' if new else "R"}{'h' if hot else 'c'}" for new, hot in zip(is_new, is_hot))
                        + "_" + torsion_type + f"_{h_str}_{sum(is_hot)}_{scaling:.2f}_{rest2_n_is_hot}")
        else:
            raise ValueError("Either is_old or is_new should be provided.")
        return t_string
    ...
    def _handle_torsion_force_terms(self, scale_dihe:dict):
        ...
        """
        # 1. Extract torsion parameters from old and new systems
        old_system_torsion_force = self._old_system_forces['PeriodicTorsionForce']
        new_system_torsion_force = self._new_system_forces['PeriodicTorsionForce']
        ...
        # 2. Static Torsion goes to standard_torsion_dict, Alchemical/REST2 Torsion goes to custom_torsion_dict
        ...
        # save the hybrid torsion classification info, so that we can debug.
        ...
        # For torsions that appear in both old and new systems (intersection), they will be scaled by REST2 or keep static.
        for torsion_key, paramAB in itersection_dict.items():
            ...
            is_core_env = [idx in self._atom_classes["core_atoms"] or idx in self._atom_classes["environment_atoms"] for idx in index_list]
            assert sum(is_core_env) == 4, f"{index_list} (hybrid 0-index), All atoms in a intersect torsion should be in core or environment regions."
            ...
            if sum(is_hot) in [ 1, 2, 3, 4 ]: # at least one hot atom
                if torsion_type_old in [ "normal", "double"]:
                    ...
                    self.hybrid_torsion_dict["intersection"][torsion_key].append(
                        self._torsion_str(is_old=[False]*4, is_hot=is_hot, hybird=True, torsion_type=torsion_type_old,
                                          scaling=1.0, rest2_n_is_hot=sum(is_hot))
                    ) # 4 real atoms with hot/cold info
                ...
                self.hybrid_torsion_dict["intersection"][torsion_key].append(
                    self._torsion_str(is_old=[False]*4, is_hot=is_hot, hybird=False, torsion_type=torsion_type_old,
                                          scaling=1.0, rest2_n_is_hot=0)
                )
            else:
        ...
        for torsion_key, param in old_only_dict.items():
            ...
            if sum(is_old) >= 1: # this is a torsion including dummy particle, optionally scale to 0
                if torsion_type in ["normal", "double"]:
                    ...
                    scaling = scale_dihe[periodicity]
                    custom_torsion_dict[torsion_key] = [periodicity, phase, k,
                                                        periodicity, phase, k * scaling,
                                                        sum(is_hot)]
                    self.hybrid_torsion_dict["old_only"][torsion_key].append(
                        self._torsion_str(is_old=is_old, is_hot=is_hot, hybird=True, torsion_type=torsion_type,
                                          scaling=scaling, rest2_n_is_hot=sum(is_hot))
                    )

                elif torsion_type == "improper":
                    # rest2 scaling + scaling in dummy state
                    scaling = scale_dihe[f"i{periodicity}"]
                    custom_torsion_dict[torsion_key] = [periodicity, phase, k,
                                                        periodicity, phase, k * scaling,
                                                        sum(is_hot)]
                    self.hybrid_torsion_dict["old_only"][torsion_key].append(
                ...
                if torsion_type in ["normal", "double", "improper"]:
                    ...
                    self.hybrid_torsion_dict["old_only"][torsion_key].append(
                        self._torsion_str(is_old=is_old, is_hot=is_hot, hybird=True, torsion_type=torsion_type,
                                          scaling=0.0, rest2_n_is_hot=sum(is_hot))
                    )
                else:
                    raise ValueError(f"The type of this old only torsion cannot be understood. {index_list}")
        ...
        for torsion_key, param in new_only_dict.items():
            ...
            if sum(is_new) >= 1: # this is a torsion including dummy particle, optionally scale to 0
                if torsion_type in ["normal", "double"]:
                    # rest2 scaling + scaling in dummy state
                    scaling = scale_dihe[periodicity]
                    custom_torsion_dict[torsion_key] = [periodicity, phase, k * scaling,
                                                        periodicity, phase, k,
                                                        sum(is_hot)]
                    self.hybrid_torsion_dict["new_only"][torsion_key].append(
                        self._torsion_str(is_new=is_new, is_hot=is_hot, hybird=True, torsion_type=torsion_type,
                                          scaling=scaling, rest2_n_is_hot=sum(is_hot))
                    )
        ...
        # 3. Add torsion into corresponding forces
    ...
    def _handle_nonbonded(self):
        """
        Handle the nonbonded interactions defined in the new and old systems.
        k_rest2 = T_cold / T_hot

        Coulomb
        -------
        1. Old atoms (atoms that only show in state A).
            chg = chgA * (1-λ) * k_rest2_sqrt
            One offsets are needed:

        2. New atoms (atoms that only show in state B).
            chg = chgB * λ * k_rest2_sqrt
        ...
        4. EnvH atoms (atoms that are not alchemically modified, but included in hot region).
            chg = chg * k_rest2_sqrt
            One offset is needed:

        5. remove all 1-4 involving hot atoms

        +--------+------------------------------+----------------------+-------------+
        | X      | param                        | math                 | chargeScale |
        +========+==============================+======================+=============+
        | Old    | lam_ele_del_x_k_rest2_sqrt   | (1-λ) * k_rest2_sqrt | chgA        |
        +--------+------------------------------+----------------------+-------------+
        | New    | lam_ele_ins_x_k_rest2_sqrt   | λ     * k_rest2_sqrt | chgB        |
        +--------+------------------------------+----------------------+-------------+
        | Core   | lam_ele_coreA_x_k_rest2_sqrt | (1-λ) * k_rest2_sqrt | chgA        |
        +--------+------------------------------+----------------------+-------------+
        | Core   | lam_ele_coreB_x_k_rest2_sqrt | λ     * k_rest2_sqrt | chgB        |
        +--------+------------------------------+----------------------+-------------+
        | EnvH   | k_rest2_sqrt                 | k_rest2_sqrt         | chg         |
        +--------+------------------------------+----------------------+-------------+
        ...
        # Define new global parameters for NonbondedForce
        self._hybrid_system_forces['standard_nonbonded_force'].addGlobalParameter('lam_ele_coreA_x_k_rest2_sqrt', 1.0)
        self._hybrid_system_forces['standard_nonbonded_force'].addGlobalParameter('lam_ele_coreB_x_k_rest2_sqrt', 0.0)
        ...
        for particle_index in range(self._hybrid_system.getNumParticles()):
            ...
            if particle_index in self._atom_classes['unique_old_atoms']:
                ...
                # Add particle to the regular nonbonded force, for coulomb
                check_index = self._hybrid_system_forces['standard_nonbonded_force'].addParticle(
                    0.0*charge, sigma, 0.0*epsilon
                )
                _check_indices(particle_index, check_index)
                ...
                if particle_index not in self._atom_classes['rest2_atoms']:
                    raise ValueError(f"Unique new atom {particle_index} must be in the rest2 region.")
                # Get the parameters in the new system
                new_index = hybrid_to_new_map[particle_index]
                [charge, sigma, epsilon] = new_system_nonbonded_force.getParticleParameters(new_index)
                ...
                # Lennard-Jones will be handled by CustomNonbondedForce
                check_index = self._hybrid_system_forces['standard_nonbonded_force'].addParticle(
                    0.0*charge, sigma, 0.0
                )
                _check_indices(particle_index, check_index)

                # lambda_electrostatics_insert ∈ [0,1]
                # chg = chgB *  λ * k_rest2
                ...
                if particle_index not in self._atom_classes['rest2_atoms']:
                    raise ValueError(f"Core atom {particle_index} must be in the rest2 region.")
                # Get the parameters in the new and old systems:
                old_index = hybrid_to_old_map[particle_index]
                [charge_old, sigma_old, epsilon_old] = old_system_nonbonded_force.getParticleParameters(old_index)
                new_index = hybrid_to_new_map[particle_index]
                ...
                # standard_nonbonded and zero sterics
                check_index = self._hybrid_system_forces['standard_nonbonded_force'].addParticle(
                    0.0*charge_old, 0.5*(sigma_old+sigma_new), 0.0)
                _check_indices(particle_index, check_index)
                ...
                # lambda_electrostatics_core ∈ [0,1]
                # chg = (chgA * (1-λ) + chgB * λ) * k_rest2
    ...
    def _atom_group(self, hybrid_index):
        if hybrid_index in self._atom_classes['core_atoms']:
            return "core"
        elif hybrid_index in self._atom_classes['unique_new_atoms']:
            return "new"
    ...
    def _handle_original_exceptions(self):
        ...
        All the exceptions including rest2 atoms will be handled by a CustomBondedForce. Only the Env_cold-Env_cold
        exception will be handled by the NonbondedForce.
        """
        # Get what we need to find the exceptions from the new and old systems:
        old_system_nonbonded_force = self._old_system_forces['NonbondedForce']
        new_system_nonbonded_force = self._new_system_forces['NonbondedForce']
    ...
    def _handle_old_new_exceptions(self):
        ...
        * Move old_new_bond_exceptions to a dictionary or similar.
        ...
        old_new_nonbonded_exceptions = "U_electrostatics + U_sterics*k_rest2_sqrt^2;"
        ...
        old_new_nonbonded_exceptions += "U_electrostatics = (lam_ele_ins_x_k_rest2_sqrt * unique_new + unique_old * lam_ele_del_x_k_rest2_sqrt) * ONE_4PI_EPS0*chargeProd/r;"
        old_new_nonbonded_exceptions += "ONE_4PI_EPS0 = %f;" % ONE_4PI_EPS0
